...
async def fetch_birthday_gif() -> str | None:
    """Busca un GIF de cumpleaños en KLIPY. Retorna URL o None si falla."""
    if not KLIPY_KEY:
        return None
    try:
...
async def send_birthday_message(channel: discord.TextChannel, member: discord.Member):
    ...
    embed   = discord.Embed(description=mensaje, color=discord.Color.from_str("#ff6eb4"))
...
async def check_birthdays():
    ...
    tmrw     = ((now + timedelta(days=1)).month, (now + timedelta(days=1)).day)
    ...
    for guild in client.guilds:
        ...
        channel    = (
            ...
            else next((c for c in guild.text_channels if c.permissions_for(guild.me).send_messages), None)
        )
        if not channel:
            continue
...
# ── /setcanal ─────────────────────────────────────────────────────────────────
@client.tree.command(name="setcanal", description="[Admin] Define el canal donde se anuncian los cumpleaños")
...
async def slash_delete_birthday(interaction: discord.Interaction):
    ...
    user_id = str(interaction.user.id)
    if user_id not in gdata["birthdays"]:
        await interaction.response.send_message("No tenés ningún cumpleaños guardado.", ephemeral=True)
        return
    del gdata["birthdays"][user_id]
    save_data(data)
    await interaction.response.send_message("🗑️ Tu cumpleaños fue eliminado.", ephemeral=True)
...
async def slash_list_birthdays(interaction: discord.Interaction):
    ...
    entries  = sorted(birthdays.items(), key=lambda x: datetime.strptime(x[1]["date"], "%d/%m/%Y").strftime("%m%d"))
    lines    = []
...
# ── /proximoscumples ──────────────────────────────────────────────────────────
@client.tree.command(name="proximoscumples", description="Muestra los próximos 5 cumpleaños del servidor")
async def slash_upcoming(interaction: discord.Interaction):
    ...
    upcoming = sorted(birthdays.items(), key=lambda x: days_until(x[1]["date"]))[:5]
    lines    = []
...
# ── /testcumple (admin) ───────────────────────────────────────────────────────
@client.tree.command(name="testcumple", description="[Admin] Simula el aviso de cumpleaños para hoy")
@app_commands.default_permissions(administrator=True)
async def slash_test_birthday(interaction: discord.Interaction):
    ...
    msg = "✅ Mensajes enviados." if found else "📭 No hay cumpleaños hoy. Guardá el tuyo con `/cumple` usando la fecha de hoy y probá de nuevo."
